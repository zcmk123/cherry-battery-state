...
- 充电状态秒级切换（约 1 秒）：轻量 USB 监视线程检测插拔充电线后立即刷新，不产生额外无线流量，也无需缩短轮询间隔
- 低电量自动提醒（≤20%，充电时不提醒）
- 休眠检测（键盘闲置时图标变暗）
...
5. 独立监视线程每秒枚举一次 USB 设备（纯本地枚举，不打开设备）。插上充电线后键盘立即以有线模式（新 PID）枚举，拔线即消失，监视线程据此立刻唤醒轮询刷新，充电状态约 1 秒内切换，无需等待下一个轮询周期
